...
extension Textures {
    ...
    private func textureOptions(for semantic: MDLMaterialSemantic,
    ...
    private func texture(from mdlTexture: MDLTexture?,
    ...
    private func properties(with semantic: MDLMaterialSemantic) -> [MDLMaterialProperty] {
        material?.properties(with: semantic) ?? []
    }

    private func textureProperty(with semantic: MDLMaterialSemantic) -> MDLMaterialProperty? {
        properties(with: semantic).first { $0.textureSamplerValue?.texture != nil }
    }

    private func scalarProperty(with semantic: MDLMaterialSemantic) -> MDLMaterialProperty? {
        properties(with: semantic).first { $0.textureSamplerValue?.texture == nil }
    }

    private func scalarFactor(for semantic: MDLMaterialSemantic,
    ...
    private func colorFactor(for semantic: MDLMaterialSemantic,
    ...
    private func colorFactor(from property: MDLMaterialProperty,
                             default defaultValue: SIMD3<Float>) -> SIMD3<Float> {
    ...
    private func baseColorFactor() -> SIMD3<Float> {
    ...
    private func isModelIODefaultBaseColorFactor(_ property: MDLMaterialProperty) -> Bool {
    ...
    private func opacityTexture() -> MTLTexture? {
    ...
    private func usesBaseColorAlpha(hasSeparateOpacityTexture: Bool) -> Bool {
    ...
    private func texturesShareSource(_ lhs: MDLTexture, _ rhs: MDLTexture) -> Bool {
        if lhs === rhs {
            return true
        }
    ...
    private func textureURLIdentifier(_ texture: MDLTexture) -> String? {
    ...
    private func texturesHaveMatchingProbe(_ lhs: MDLTexture, _ rhs: MDLTexture) -> Bool {
    ...
    private func textureHasVisibleAlpha(_ mdlTexture: MDLTexture) -> Bool {
    ...
    private func normalizedRGBAProbe(for mdlTexture: MDLTexture,
    ...
    private func texturePropertyValue(with semantic: MDLMaterialSemantic) -> MTLTexture? {
